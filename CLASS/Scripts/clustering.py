...
class CourseClusterer:
    ...
    def __init__(self, n_clusters=5, random_state=42, auto_clusters=False, max_clusters=10, config=None, dataset=None):
        ...
        # Rarity bonus (soft, optional)
        self.use_rarity_bonus = bool(config.get('use_rarity_bonus', True)) if config else True
        self.rarity_alpha = float(config.get('rarity_alpha', 0.15)) if config else 0.15

        #self.course_rarities = dataset.get_course_rarities()
        self.skill_rarities = dataset.get_skill_rarities()
    ...
    def adjust_reward(self, course_idx, original_reward, prev_reward, actual_skills):
        ...
        """
        # --- Apply rarity bonus if enabled ---
        if getattr(self, "use_rarity_bonus", False):
            course_rarities = self.dataset.compute_course_rarities_2(actual_skills)
            rarity = course_rarities[course_idx]
            original_reward *= (1.0 + self.rarity_alpha * rarity)
        ...
        elif self.dataset.config['version'] in ['v1', 'v2']:
        ...
        if reward_change > 0:  # Better than best adjusted reward so far
            ...
            if current_cluster == self.prev_cluster:  # Same cluster
                ...
                adjusted_reward = original_reward * self.reward_multipliers['diff_cluster_decrease']  # 0.7
            return adjusted_reward
