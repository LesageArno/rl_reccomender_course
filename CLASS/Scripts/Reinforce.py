...
class Reinforce:
    ...
    def __init__(
        ...
        self.model_filename = f"{self.model_name}_k_{self.k}_run_{run}"
    ...
    def reinforce_recommendation(self):
        ...
        save_dir = self.dataset.config.get("save_dir", "CLASS")
        model_dir = os.path.join(save_dir, "models_weights")
        os.makedirs(model_dir, exist_ok=True)
        self.model.save(os.path.join(model_dir, self.model_filename))
            ...
            while not done:
                if self.model_name == "ppo_mask":
                    if hasattr(self.eval_env, "envs"):
                        print("CE L'HO")
                        print(self.eval_env.envs)
                        mask = self.eval_env.envs[0].get_action_mask()
                    else:
                        #print("ECCOMI")
                    ...
                    obs = self.eval_env.get_obs()  # return _agent_skills which is current state
