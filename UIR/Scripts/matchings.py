...
def compute_coverage_percentage(learner: np.ndarray, jobs: np.ndarray) -> float:
    #print("These are jobs", jobs[0])
    #print("These are learner", learner)
    percentages = np.zeros(len(jobs))

    for i, job in enumerate(jobs):
        match = matching(learner, job)
        percentages[i] = match
...
def action_reward(
    ...
    prev_matchings = matches_array(prev_skills, jobs)
    prev_matchings[prev_matchings < soft_threshold] = 0
    prev_matchings[prev_matchings >= soft_threshold] = 1

    nb_prev_potential = np.sum(prev_matchings, axis=0)
    ...
    nb_skill_acquired = np.count_nonzero(actual_skills - prev_skills)
    penalty_2 = 1 / (1 + nb_skill_acquired * (nb_new_applicable_jobs + nb_potential_applicable_jobs*0.5))
    # print(f"nb_skill_acquired: {nb_skill_acquired}")

    #reward = nb_new_applicable_jobs
    P = max(nb_prev_potential, 1e-6)
    w = max(P - nb_new_applicable_jobs, 0) / P  # ∈ [0,1]
    penalty = potential_penalty * np.sqrt(w)  # crescita lenta
    '''wasted_potential = max(nb_potential_applicable_jobs - nb_new_applicable_jobs,0)
    penalty = potential_penalty * wasted_potential'''
    #print(f"You wasted: {wasted_potential}")
    if not is_last:
        reward = alpha*actual_nb_applicable_jobs + beta*nb_new_applicable_jobs + gamma*(nb_potential_applicable_jobs-nb_prev_potential) - penalty - penalty_2
    else:
        reward = alpha * actual_nb_applicable_jobs + beta * nb_new_applicable_jobs - penalty - penalty_2
